#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Wrapper script to invoke pyhera-based scripts.

Run scripts with pyhera in directory mode using::

    python load-pyhera.py <global options> <name_of_script> <options>
...
Currently `<global options>` are :ref:`global options`. 
"""
# (C) John Erling Blad, 2021
# (C) Pywikibot team, 2009-2021
#
# The whole file or parts of it is based upon code from pywikibot
...
def check_hera_versions(package):
    """Validate package version and scripts version.

    Rules:
        - PyHERA version must not be older than scrips version
        - Scripts version must not be older than previous PyHERA version
          due to deprecation policy
    """
    from pyhera.tools import Version
    scripts_version = Version(getattr(package, '__version__', hera.__version__))
    hera_version = Version(hera.__version__)

    if scripts_version.release > hera_version.release:
        print('WARNING: PyHERA version {} is behind scripts package '
              'version {}.\nYour PyHERA may need an update or be '
              'misconfigured.\n'.format(hera_version, scripts_version))

    # calculate previous minor release
    prev_hera_version = Version('{v.major}.{}.{v.micro}'
                           .format(hera_version.minor - 1,
                                   v=hera_version,))

    if scripts_version.release < prev_hera_version.release:
        print('WARNING: Scripts package version {} is behind legacy PyHERA '
              'version {} and current version {}\nYour scripts may need an '
              'update or be misconfigured.\n'
              .format(scripts_version, prev_hera_version, hera_version, ))
    del Version
...
def run_python_file(filename, argv, argvu, package=None):
    ...
    if package:
        ...
        check_hera_versions(package)

    # Set sys.argv and the first path element properly.
    old_argv = sys.argv
    old_argvu = hera.argvu
...
# todo: is this used?
def handle_args(hera_py, *args):
    """Handle args and get filename.

    :return: filename, script args, local args for hera.py
    :rtype: tuple
    """
    fname = None
    index = 0
...
def _print_requirements(requirements, script, variant):
    ...
    print(format_string.format(script or 'pyhera', variant))
...
# Use env var to communicate to config.py where load-pyhera.py is located.
_hera_dir = os.path.split(__file__)[0]
os.environ['PYHERA_DIR_LOAD'] = _hera_dir
try:
    import pyhera as hera
    print(hera.__dict__)
except RuntimeError:
    os.environ['PYHERA_NO_USER_CONFIG'] = '2'
    import pyhera as hera
    ...
    if filename is not None and not (filename.startswith('generate_')
        ...
        run_python_file(os.path.join(_hera_dir, 'generate_user_files.py'),
                        ['generate_user_files.py'],
                        ['generate_user_files.py'])
        # because we have loaded pyhera without user-config.py loaded,
...
def find_filename(filename):
    ...
    for file_package in script_paths:
        ...
        testpath = os.path.join(_hera_dir, *paths)
...
def main():
    ...
    if global_args:  # don't use sys.argv
        unknown_args = hera.handle_args(global_args)
        if unknown_args:
            print('ERROR: unknown load-pyhera.py argument{}: {}\n'
    ...
    # When both load-pyhera.py and the filename to run are within the current
    # working directory:
